 ...
 */
public class TodoListPreact extends BasePage
{
	/**
	 * The todo object.
	 */
	...
	private class AddItemsContainer extends WebMarkupContainer
	{
		...
		void onShowForm(Optional<AjaxRequestTarget> targetOptional)
		{
			...
			targetOptional.ifPresent(target -> target.add(PreactReplacementEnablingBehavior.PREACT, this));
		}
		...
		void onRemoveCompletedTodos(Optional<AjaxRequestTarget> targetOptional)
		{
			...
			targetOptional.ifPresent(target -> {
				// repaint our panel
				target.add(PreactReplacementEnablingBehavior.PREACT, this);

				// repaint the listview as there was a new item added.
				target.add(PreactReplacementEnablingBehavior.PREACT, showItems);
			});
		}
	...
	public void renderHead(IHeaderResponse response)
	{
		...
		response.render(
			CssHeaderItem.forReference(new CssResourceReference(TodoListPreact.class, "todo.css")));
	}
}
