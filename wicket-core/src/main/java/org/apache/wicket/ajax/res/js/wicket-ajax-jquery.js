...
;(function (jQuery, undefined) {
	...
	// Copied from https://github.com/epoberezkin/fast-deep-equal 3.1.3
	...
	Wicket.Ajax.Call.prototype = {
		...
		doAjax: function (attrs) {
			...
			if (attrs.bdi) {
				var element = (attrs.bddc && !jQuery.isWindow(attrs.bddc)) ?
					Wicket.$(attrs.bddc) :
					((attrs.c && !jQuery.isWindow(attrs.c)) ?
						Wicket.$(attrs.c) :
						null);
				if (element) {
					var blockDuplicatesPropertyName = "wicketAjaxBlockDuplicates" + attrs.bdi;
					var previouslySubmittedData = element[blockDuplicatesPropertyName];
					if (previouslySubmittedData && equal(data, previouslySubmittedData)) {
						self.done(attrs);
						return false;
					} else {
						element[blockDuplicatesPropertyName] = data;
					}
				}
			}
